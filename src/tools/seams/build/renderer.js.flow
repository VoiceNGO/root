...
class Renderer {
  ...
  decodeSeamData(data: string) {
    ...
    const { number: width, offset: widthOffset }: { width: number, widthOffset: number } = intDecode(bytes, 1);
    const { number: height, offset: heightOffset }: { height: number, heightOffset: number } = intDecode(
      bytes,
      widthOffset,
    );
    const { number: numSeams, offset: numSeamsOffset }: { numSeams: number, numSeamsOffset: number } = intDecode(
      bytes,
      heightOffset,
    );
