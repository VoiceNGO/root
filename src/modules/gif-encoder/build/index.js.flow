...
export type tColorTable = Array<tColor>;
export type tDisposalMethods = $Values<typeof DISPOSAL_METHODS>;
export type tColorWeightSignature = (r1: number, g1: number, b1: number, r2: number, g2: number, b2: number) => number;
...
const COLOR_WEIGHT_METHODS: { [string]: tColorWeightSignature } = {
  perceptive: (r1, g1, b1, r2, g2, b2) =>
    Math.pow((r2 - r1) * 0.3, 2) + Math.pow((g2 - g1) * 0.59, 2) + Math.pow((b2 - b1) * 0.11, 2),
...
export default class GIFCanvas extends Canvas {
  ...
  colorTable: ?tColorTable = null;
  currentFrame: GIFFrame;
  currentFrameIndex: number = -1;
  delay: number = 33;
  repeat: boolean = true;
  colorWeightMethod: tColorWeightSignature = COLOR_WEIGHT_METHODS.perceptive;
  disposalMethod: tDisposalMethods = DISPOSAL_METHODS.DO_NOT_DISPOSE;
  ...
  getHeaderBytes(): Array<number> {
    const colorTableSize = this.colorTable
      ? { '256': 7, '128': 6, '64': 5, '32': 4, '16': 3, '8': 2, '4': 1 }[this.colorTable.length]
      : 0;
  ...
  getColorTable(): ?tColorTable {
    return this.colorTable;
  }

  setColorTable(table: tColorTable): this {
...
class GIFFrame {
  ...
  serialize(): Array<number> {
    ...
    const imageDescriptor = [
      ...
      (localColorFlag << 7) | (interlaceFlag << 6) | (sortFlag << 5) | colorTableSize,
    ];
  }
}
