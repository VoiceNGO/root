...
const app = new Program()
  ...
  .addHelp('The seams CLI is only used to generate seam files.')
  .addHelp('Please see the readme for more detailed explanations of options.')
  .example('seamgen -c -s=2 -o=/path/to/out/directory /path/to/images/**/*.jpg')
  .option('--compression, -c', 'Enables seam compression')
  .option('--spacing, -c <Number>', 'Sets spacing level')
  ...
  Promise.all(globs)
    ...
    .map(function(file) {
      ...
      new Generator(file)
        ...
        .setCompression(args.compression)
        .setSpacing(args.spacing)
        .setMerging(args.merging)
