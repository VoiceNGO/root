...
var _encode = require('vle-integers/encode');

var _matrices = require('./matrices');

var _matrices2 = _interopRequireDefault(_matrices);

var _timerDecorator = require('timer-decorator');

var _timerDecorator2 = _interopRequireDefault(_timerDecorator);
 ...
 */
function toDenseArray(seams, stepSize) {
  return seams.map(seam => [].concat.apply([], seam.map((val, ix, ary) => Array.apply(0, Array(ix < ary.length - 1 ? stepSize : 1)).map((a, aIx) => val + (val < ary[ix + 1] ? 1 : -1) * aIx))));
}
...
let Generator = (_class = class Generator {
  /**
  ...
  constructor(imgPromise) {
    ...
    // set defaults
    this.setCompression(false).setSpacing().setMerging().setDirection().setPercentage();
  }
  ...
  generateEnergyMap() {
    ...
    function energyAtPixel(x, y, pxData, rows, cols) {
      ...
      left && (xSum += -2 * pxEnergy(pxData, cols, x - 1, y), up && (xSum += tmp = -1 * pxEnergy(pxData, cols, x - 1, y - 1), ySum += tmp), down && (xSum += -1 * (tmp = pxEnergy(pxData, cols, x - 1, y + 1)), ySum += tmp));

      right && (xSum += 2 * pxEnergy(pxData, cols, x + 1, y), up && (xSum += tmp = pxEnergy(pxData, cols, x + 1, y - 1), ySum += -1 * tmp), down && (xSum += tmp = pxEnergy(pxData, cols, x + 1, y + 1), ySum += tmp));
  ...
  gradientMagnitude(x, y) {
    const cols = this.width;
    const rows = this.height;
    const matrixX = _matrices2.default.gradientMagnitudeX;
    const matrixY = _matrices2.default.gradientMagnitudeY;
    let xSum = 0;
    let ySum = 0;
    ...
    for (let row = y ? -1 : 0, limit = y < rows ? 2 : 1; row < limit; row++) {
      ...
      xSum += (!(tmp = matrixX[row + 1][0]) ? 0 : tmp * this.getPixelSubEnergy(left, y + row)) + (!(tmp = matrixX[row + 1][1]) ? 0 : tmp * this.getPixelSubEnergy(center, y + row)) + (!(tmp = matrixX[row + 1][2]) ? 0 : tmp * this.getPixelSubEnergy(right, y + row));

      ySum += (!(tmp = matrixY[row + 1][0]) ? 0 : tmp * this.getPixelSubEnergy(left, y + row)) + (!(tmp = matrixY[row + 1][1]) ? 0 : tmp * this.getPixelSubEnergy(center, y + row)) + (!(tmp = matrixY[row + 1][2]) ? 0 : tmp * this.getPixelSubEnergy(right, y + row));
    }
  ...
  createSeam(debug) {
    ...
    for (let i = this.height - stepSize; i > -stepSize; i -= stepSize) {
      ...
      let nextSteps = [sumAdjoiningVals(energyRow, center, mergeSize, -1, width), compressed ? Infinity : 0, sumAdjoiningVals(energyRow, center, mergeSize, +1, width)];
  ...
  generateSeams() {
    ...
    return this.imgPromise.then(this.setRotation.bind(this)).then(this.generateEnergyMap.bind(this)).then(this.computeLowestSeamEnergies.bind(this)).then(this.__generateSeams.bind(this));
  }
   ...
   */
  encode() {
    return this.getSeamData(true).then(this.__encode.bind(this));
  }
  ...
  __encode(seamData) {
    ...
    bytes.push.apply(bytes, [].concat(
    // flattens the following data
    this.stepSize << 4 + ( // <4 bits> step size
    this.mergeSize - 1 << 2) + ( // <2 bits> merge size
    this.vertical ? 0 : 1) << 1 + ( // <1 bit>  vertical or horizontal
    this.compressed ? 1 : 0), // <1 bit>  compressed or not
    (0, _encode.array)(this.width), // <varInt> width
    (0, _encode.array)(this.height), // <varInt> height
    (0, _encode.array)(this.numSeams) // <varInt> # of seams
    ));
    ...
    seamData.seams.forEach(function (seam) {
      ...
      bytes.push.apply(bytes, (0, _encode.array)(seam[0]));
      ...
      for (let i = 1, l = seam.length; i < l; i++) {
        ...
        let bits = val < prev ? [0] // there are only 3 possible values, so we can always encode one with a single bit
...
}, (_applyDecoratedDescriptor(_class.prototype, 'generateEnergyMap', [_timerDecorator2.default], Object.getOwnPropertyDescriptor(_class.prototype, 'generateEnergyMap'), _class.prototype), _applyDecoratedDescriptor(_class.prototype, 'computeLowestSeamEnergies', [_timerDecorator2.default], Object.getOwnPropertyDescriptor(_class.prototype, 'computeLowestSeamEnergies'), _class.prototype), _applyDecoratedDescriptor(_class.prototype, '__generateSeams', [_timerDecorator2.default], Object.getOwnPropertyDescriptor(_class.prototype, '__generateSeams'), _class.prototype), _applyDecoratedDescriptor(_class.prototype, '__encode', [_timerDecorator2.default], Object.getOwnPropertyDescriptor(_class.prototype, '__encode'), _class.prototype)), _class);
