...
class EventEmitter {
  ...
  addListener(event, listener, scope) {
    let [eventName, handler] =
      this.single && !~DEF_EVENTS.indexOf(` ${eventName} `)
        ? [DEFAULT, event]
        : [event, listener];
  ...
  removeListener(event, listener) {
    ...
    handlers = handlers.filter(function (handlerObj) {
      if (handlerObj.handler === handler) {
        this.__emit(
          'removeListener',
          (this.single ? [] : [eventName]).concat(handlerObj.handler)
        );
        return false;
      }
  ...
  listeners(event) {
    return (this.events[event] || []).map(function (handler) {
      return handler.handler;
    });
  }

  emit(event) {
    return this.__emit.apply(
      this,
      this.single ? [DEFAULT, ...arguments] : arguments
    );
  }
}
  ...
  addHandler: {
    value: function (eventName, handler) {
      ...
      if (
        ...
        (!this.warned || !this.warned[eventName])
      ) {
        console.warn(
