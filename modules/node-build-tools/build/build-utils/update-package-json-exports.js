...
export async function getFolderMapping(projectFolder, filesToInclude) {
...
export async function getUpdatedJson(projectFolder) {
    const packageJsonSrc = join(projectFolder, packageJSONFileName);
    const [folderMappings, packageJson] = await genAllEnforce(getFolderMapping(projectFolder), readJsonFile(packageJsonSrc, packageJSONFileName));
    packageJson.exports = folderMappings;
    return packageJson;
}
export default async function updatePackageJsonExports(projectFolder) {
    const packageJsonSrc = join(projectFolder, packageJSONFileName);
